...
function DeleteContent(props) {
  ...
  const handleDelete = async () => {
    if (props.type === "deleteForum" && state?.user?.id === +idUser) {
      await axios
        .delete(`http://docker-alb-be-1095592879.us-west-2.elb.amazonaws.com:9090/forums/${state?.id}`, {
          headers: {
      ...
      await axios
        .delete(`http://docker-alb-be-1095592879.us-west-2.elb.amazonaws.com:9090/blogs/${state?.id}`, {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${key}`,
          },
