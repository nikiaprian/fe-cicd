...
function LikeUnlikeAnswer(props) {
  ...
  const handleLike = async () => {
    if (state?.status_like) {
      await axios
        .delete(`http://docker-alb-be-1664573111.us-west-2.elb.amazonaws.com:9090/like/forum/comment/${state?.id}`, {
          headers: {
      ...
      await axios
        .post(
          `http://docker-alb-be-1664573111.us-west-2.elb.amazonaws.com:9090/like/forum/comment/${state?.id}`,
          {},
          {
            headers: {
              "Content-Type": "application/json",
